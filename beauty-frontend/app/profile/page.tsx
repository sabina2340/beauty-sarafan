...
export default function ProfilePage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setProfile(saved);
      setSuccess("Профиль отправлен на модерацию");
    } catch (err) {
  ...
  return (
    <section className="card authCard">
      ...
      <div className="profileStatus">
        {profile ? (
          <>
            <strong>Статус карточки мастера: {profile.status}</strong>
            {profile.rejection_reason ? <p className="adminError">Причина отклонения: {profile.rejection_reason}</p> : null}
          </>
        ) : (
      ...
      <form className="authForm" onSubmit={onSubmit}>
        ...
        <textarea id="services" className="textarea" value={services} onChange={(e) => setServices(e.target.value)} />
        ...
        <input id="social" className="input" value={socialLinks} onChange={(e) => setSocialLinks(e.target.value)} />
