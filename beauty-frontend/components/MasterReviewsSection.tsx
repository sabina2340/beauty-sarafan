...
import { createMasterReview, getMasterReviews } from "@/lib/api";
...
export function MasterReviewsSection({ masterId, initialReviews }: Props) {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setPhoto(null);
      setConsent(false);
      const freshReviews = await getMasterReviews(masterId).catch(() => reviews);
  ...
  return (
    <article className="card reviewsSection">
      ...
      <div className="reviewsList">
        {reviews.length === 0 ? <p className="muted">Пока нет опубликованных отзывов.</p> : null}
        {reviews.map((review) => (
