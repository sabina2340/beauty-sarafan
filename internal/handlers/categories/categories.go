...
// @Tags categories
// @Produce json
// @Param audience query string false "Фильтр по аудитории: master|client"
// @Success 200 {array} models.Category
// @Router /categories [get]
func List(c *gin.Context) {
	var items []models.Category

	audience := c.Query("audience") // master | client | empty

	query := database.DB.Order("id asc")

	if audience == "master" || audience == "client" {
		query = query.Where("audience = ? OR audience = ?", audience, "both")
	}
